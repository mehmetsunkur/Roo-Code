## release the current code changes as docker image
## builds docker image tagged aos/aos-vsce with the vsix file './bin/aos-code-*.vsix'
...
if (( ${#files[@]} )); then
  ...
  echo "Error! No matching file found: './bin/aos-code-*.vsix'"
  exit 1
fi
